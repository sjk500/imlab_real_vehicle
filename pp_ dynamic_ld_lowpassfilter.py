...
filtered_steering_angle = 0  # Low-pass filter 초기값

FILTER_ALPHA = 0.8  # 조향각 smoothing 계수
...
# ========== Lookahead 거리 ==========
def compute_lookahead_dist(speed):
    k1 = 1.2
    k2 = 2.0
    return max(k1 * speed + k2, 2.5)
...
# ========== Pure Pursuit ==========
def pure_pursuit_control(path_enu, current_pos, current_yaw, current_speed):
    x_gnss, y_gnss = current_pos
    x_rear = x_gnss - GNSS_OFFSET * np.cos(current_yaw)
    y_rear = y_gnss - GNSS_OFFSET * np.sin(current_yaw)

    lookahead_dist = compute_lookahead_dist(current_speed)
    ...
    return int(np.degrees(delta) * 20)
...
if __name__ == '__main__':
    try:
        ...
        print("[시작] 자율조향 + 수동 엑셀/브레이크 제어 + Filter 적용")
        ...
        while True:
            ...
            raw_angle = pure_pursuit_control(path_enu, (x_gnss, y_gnss), current_yaw_rad, current_speed)

            # Low-pass filter 적용
            filtered_steering_angle = (
                FILTER_ALPHA * filtered_steering_angle + (1 - FILTER_ALPHA) * raw_angle
            )
            target_steering_angle = int(filtered_steering_angle)
